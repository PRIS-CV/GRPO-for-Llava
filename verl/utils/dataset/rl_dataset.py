...
def convert_chat(chat):
    ...
    for i, data in enumerate(chat):
        if data["role"] == "user":
            prompt += f"<s>USER: {data['content']}</s>\n"
        elif data["role"] == "system":
            prompt += f"<s>SYSTEM: {data['content']}</s>\n"
        elif data["role"] == "assistant":
            prompt += f"<s>ASSISTANT: {data['content']}</s>\n"
...
class LlavaDataset(RLHFDataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        self.dataframe = self.dataframe[self.dataframe.apply(lambda doc: len(
            tokenizer.tokenize(convert_chat(doc[prompt_key]).replace("<image>", "<image>"*576))) <= self.max_prompt_length,
                                                             axis=1)]
